/* stream_ref_func.c
   vector streaming benchmarks - C reference implementation based on a function
   M. Bernreuther <bernreuther@hlrs.de>

	gcc -Wall -g -lm stream_ref_func.c -o stream_ref_func.gcc
	gcc -march=native -O3 -ftree-vectorizer-verbose=3 -lm stream_ref_func.c -o stream_ref_func.gcc
	//  (march=corei7-avx,sandybridge,ivybridge,haswell,... for older gcc versions)
	//  -mavx -mavx2

	clang -Wall -g -lm stream_ref_func.c -o stream_ref_func.clang
	clang -mavx2 -Ofast -lm stream_ref_func.c -o stream_ref_func.clang

	icc -Wall -fast -opt-report -lm stream_ref_func.c -o stream_ref_func.icc
	icc -fast -opt-report -parallel -par-report -vec_report3 -guide-par -guide-vec stream_ref_func.c -lm -o stream_ref_func.icc
	...
	pgcc -fast -lm stream_ref_func.c -o stream_ref_func.pgi
	craycc -O3 -hfp3 stream_ref_func.c -o stream_ref_func.cray


	//USE_PAPI: add -lpapi


	stream_ref_func [<N>] [<nrepeat>]
...
/*--------------------------------------------------------------------*/
/* C99: might use "restrict" and pointers for a,b,c */
/*inline*/ void stream(const Tindex N
           ,Tfloat a[]
           ,const Tfloat b[]
           ...
           )
{
	...
	{
		a[i]=b[i]+s*c[i];
		/* *pa=*pb+ s * *pc;  pa+=STRIDE; pb+=STRIDE; pc+=STRIDE;*/
	}
#endif
#ifdef DO_vTRIAD
	/*Tfloat *pa=a,*pb=b,*pc=c,*pd=d;*/
	for(i=0;i<N;i+=STRIDE)
	{
		a[i]=b[i]+c[i]*d[i];
